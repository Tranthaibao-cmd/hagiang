...
import { DataStateChangeEventArgs, RowDataBoundEventArgs } from '@syncfusion/ej2-angular-grids';
...
@Component({
  selector: 'app-fiveyear-plan-table',
  templateUrl: './fiveyear-plan-table.component.html',
  styleUrls: ['./fiveyear-plan-table.component.scss']
})
export class FiveyearPlanTableComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.fiveplanService.refresh$.subscribe(() => {
      this.get();
    });
    this.getUnit();
    this.get();
  ...
  update(plan) {
    ...
    this.fiveplanService.insert(plan).subscribe(
      (result) => {
        if (result.code == 200)
          Swal.fire({
            icon: 'success',
